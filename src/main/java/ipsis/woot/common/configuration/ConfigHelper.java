...
import static ipsis.woot.factory.Tier.TIER_5;
...
public class ConfigHelper {
    ...
    public static Tier getMobTier(FakeMob fakeMob, World world) {
        ...
        if (fakeMob.isValid() && world != null) {
            if (WootConfig.get().hasMobOverride(fakeMob, WootConfig.ConfigKey.MOB_TIER)) {
                ...
                if (health <= WootConfig.get().getIntConfig(WootConfig.ConfigKey.TIER_1_MAX_UNITS))
                    tier = Tier.TIER_1;
                else if (health <= WootConfig.get().getIntConfig(WootConfig.ConfigKey.TIER_2_MAX_UNITS))
                    tier = Tier.TIER_2;
                else if (health <= WootConfig.get().getIntConfig(WootConfig.ConfigKey.TIER_3_MAX_UNITS))
                    tier = Tier.TIER_3;
                else if (health <= WootConfig.get().getIntConfig(WootConfig.ConfigKey.TIER_4_MAX_UNITS))
                    tier = Tier.TIER_4;
            }
        }
        return tier;
    ...
    public static WootConfig.ConfigKey getByFactoryUpgrade(FactoryUpgradeType factoryUpgradeType, int level) {
        if (factoryUpgradeType == FactoryUpgradeType.CAPACITY) {
            if (level == 1) return WootConfig.ConfigKey.CAPACITY_1;
            if (level == 2) return WootConfig.ConfigKey.CAPACITY_2;
            if (level == 3) return WootConfig.ConfigKey.CAPACITY_3;
        } else if (factoryUpgradeType == FactoryUpgradeType.EFFICIENCY) {
