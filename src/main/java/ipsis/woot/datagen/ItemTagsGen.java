...
public class ItemTagsGen extends ItemTagsProvider {
    ...
    protected void addTags() {
        ...
        addColored(tag(Tags.Items.DYES)::addTags, Tags.Items.DYES, "{color}_dyeplate");
    }
    ...
    private void addColored(Consumer<ITag.INamedTag<Item>> consumer, ITag.INamedTag<Item> group, String pattern)
    {
        ...
        for (DyeColor dyeColor : DyeColor.values()) {
            ResourceLocation key = new ResourceLocation(Woot.MODID, pattern.replace("{color}", dyeColor.getName()));
            ITag.INamedTag<Item> tag = getForgeItemTag(prefix + dyeColor.getName());
