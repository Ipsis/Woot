...
public class InfuserBlock extends Block implements WootDebug {
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult blockRayTraceResult) {
        ...
        if (!(worldIn.getBlockEntity(pos) instanceof InfuserTileEntity))
            throw new IllegalStateException("Tile entity is missing");

        InfuserTileEntity infuser = (InfuserTileEntity)worldIn.getBlockEntity(pos);
        ...
        if (FluidUtil.getFluidHandler(heldItem).isPresent()) {
            ...
            if (infuser instanceof INamedContainerProvider)
                NetworkHooks.openGui((ServerPlayerEntity) player, infuser, infuser.getBlockPos());
            else
