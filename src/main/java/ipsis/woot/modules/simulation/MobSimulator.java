...
public class MobSimulator {
    ...
    public boolean learn(@Nonnull FakeMobKey fakeMobKey) {
        ...
        if (DropRegistry.get().isLearningFinished(fakeMobKey, SimulationConfiguration.SIMULATION_MOB_COUNT.get()))
            return false;
    ...
    public void tick(World world) {
        ...
        if (currTicks >= SimulationConfiguration.SIMULATION_TICKS_PER_SIM_TICK.get()) {
            // An actual simulation tick

            int processed = 0;
            Iterator iter = pupils.entrySet().iterator();
            while (iter.hasNext() && processed <= SimulationConfiguration.SIMULATION_CELLS_PER_SIM_TICK.get()) {
