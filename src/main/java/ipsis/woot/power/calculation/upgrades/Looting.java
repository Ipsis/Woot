...
public class Looting implements IUpgradePowerCalculator {

    @Override
    public boolean isActive(IFarmSetup iFarmSetup) {

        return iFarmSetup.hasUpgrade(EnumFarmUpgrade.LOOTING);
    }
    ...
    public BigInteger calculateUpgradeCost(IFarmSetup iFarmSetup, int spawnTicks) {
        ...
        int perTick = Woot.wootConfiguration.getInteger(
                ...
                ConfigKeyHelper.getLootingPowerPerTick(iFarmSetup.getUpgradeLevel(EnumFarmUpgrade.LOOTING)));

        BigInteger cost = BigInteger.valueOf(spawnTicks);
        cost = cost.multiply(BigInteger.valueOf(perTick));

        Woot.debugSetup.trace(DebugSetup.EnumDebugType.POWER_CALC, "looting", "upgrade cost:" + cost);
        return cost;
    }
