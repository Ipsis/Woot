...
public class FakePlayerPool {
    ...
    private static void init(WorldServer worldServer) {
        ...
        Enchantment enchantmentLooting = ForgeRegistries.ENCHANTMENTS.getValue(new ResourceLocation("looting"));
        if (enchantmentLooting == null)
            Woot.LOGGER.error("FakePlayerPool - failed to lookup enchantment looting");
